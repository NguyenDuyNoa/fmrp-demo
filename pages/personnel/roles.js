    ...
    const multiDataSet = [
        {
            columns: [
                ...
                {title: `${dataLang?.category_personnel_position_name || "category_personnel_position_name"}`, width: {wpx: 150}, style: {fill: {fgColor: {rgb: "C7DFFB"}}, font: {bold: true}}},
                {title: `${dataLang?.category_personnel_position_amount || "category_personnel_position_amount"}`, width: {wch: 30}, style: {fill: {fgColor: {rgb: "C7DFFB"}}, font: {bold: true}}},
                {title: `${dataLang?.category_personnel_position_department || "category_personnel_position_department"}`, width: {wch: 30}, style: {fill: {fgColor: {rgb: "C7DFFB"}}, font: {bold: true}}},
            ...
            <div className='px-10 xl:pt-24 pt-[88px] pb-3 space-y-2.5 h-screen overflow-hidden flex flex-col justify-between'>
                <div className='h-[97%] space-y-3 overflow-hidden'>
                    <div className='flex space-x-3 xl:text-[14.5px] text-[12px]'>
                        ...
                        <h6 className='text-[#141522]/40'>{dataLang?.header_category_personnel || "header_category_personnel"}</h6>
                        <span className='text-[#141522]/40'>/</span>
                        <h6>{dataLang?.header_category_personnel_position || "header_category_personnel_position"}</h6>
                    </div>
                    <div className='flex justify-between items-center'>
                        <h2 className='xl:text-3xl text-xl font-medium '>{dataLang?.category_personnel_position_title || "category_personnel_position_title"}</h2>
                        <div className='flex space-x-3 items-center'>
                            <Popup_ChucVu dataLang={dataLang} onRefresh={_ServerFetching.bind(this)} onRefreshSub={_ServerFetchingSub.bind(this)} className='xl:text-sm text-xs xl:px-5 px-3 xl:py-2.5 py-1.5 bg-gradient-to-l from-[#0F4F9E] via-[#0F4F9E] via-[#296dc1] to-[#0F4F9E] text-white rounded btn-animation hover:scale-105' />
                        </div>
                    </div>
                       ...
                       <div className='flex gap-2'>
                            ...
                            <div className='w-[23vw]'>
                                ...
                                <Select 
                                    ...
                                    options={[{ value: '', label: 'Chọn tên chức vụ', isDisabled: true }, ...dataPositionOption]}
                                    formatOptionLabel={CustomSelectOption}
                                    onChange={_HandleFilterOpt.bind(this, "position")}
                                    value={idPosition}
                                    noOptionsMessage={() => `${dataLang?.no_data_found}`}
                                    isClearable={true}
                                    placeholder={dataLang?.category_personnel_position_name || "category_personnel_position_name"}
                                    className="rounded-md py-0.5 bg-white border-none xl:text-base text-[14.5px] z-20" 
                                    isSearchable={true}
                                    style={{ border: "none", boxShadow: "none", outline: "none" }}
                                    theme={(theme) => ({
                        ...
                        {data.length != 0 &&
                            <div className='flex space-x-6'>
                                <ExcelFile filename={dataLang?.header_category_personnel_position || "header_category_personnel_position"} element={
                                    <button className='xl:px-4 px-3 xl:py-2.5 py-1.5 xl:text-sm text-xs flex items-center space-x-2 bg-[#C7DFFB] rounded hover:scale-105 transition'>
                                        <IconExcel size={18} />
                                        <span>{dataLang?.client_list_exportexcel}</span>
                                    </button>
                                }>
                                    <ExcelSheet dataSet={multiDataSet} data={multiDataSet} name={dataLang?.header_category_personnel_position || "header_category_personnel_position"} />
                                </ExcelFile>
                    ...
                    <div className='min:h-[500px] h-[91%] max:h-[800px] overflow-auto pb-2 scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100'>
                        <div className='xl:w-[100%] w-[110%] pr-2'>
                            <div className='flex items-center sticky top-0 bg-white p-2 z-10 shadow-[-20px_-9px_20px_0px_#0000003d]'>
                                <h4 className='w-[10%]'/>
                                <h4 className='xl:text-[14px] text-[12px] px-2 text-[#667085] uppercase w-[20%] font-[300] truncate'>{dataLang?.category_personnel_position_name || "category_personnel_position_name"}</h4>
                                <h4 className='xl:text-[14px] text-[12px] px-2 text-[#667085] uppercase w-[15%] font-[300] truncate text-center'>{dataLang?.category_personnel_position_amount || "category_personnel_position_amount"}</h4>
                                <h4 className='xl:text-[14px] text-[12px] px-2 text-[#667085] uppercase w-[20%] font-[300] truncate'>{dataLang?.category_personnel_position_department || "category_personnel_position_department"}</h4>
                            ...
                            <div className='divide-y divide-slate-200'>
                                ...
                                {onFetching && <Loading className="h-80"color="#0f4f9e" />}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className='py-4 w-[600px] space-y-5'>
                {onFetching ?
                    ...
                    <React.Fragment>
                        ...
                        <div className='space-y-1'>
                            <label className="text-[#344054] font-normal text-base">{props.dataLang?.category_personnel_position_department || "category_personnel_position_department"} <span className='text-red-500'>*</span></label>
                            <Select 
                                options={dataOptDepartment}
                                value={department ? {label: dataOptDepartment?.find(x => x.value === department)?.label, value: department} : null}
                                ...
                                className={`${errDepartment ? "border-red-500" : "border-transparent" } placeholder:text-slate-300 w-full bg-[#ffffff] rounded text-[#52575E] font-normal outline-none border `}
                                isSearchable={true}
                                theme={(theme) => ({
                                    ...theme,
                                    colors: {
                            ...
                            />
                            {errDepartment && <label className="text-sm text-red-500">{props.dataLang?.category_personnel_position_err_department || "category_personnel_position_err_department"}</label>}
                        </div>
                        <div className='space-y-1'>
                            <label className="text-[#344054] font-normal text-base">{props.dataLang?.category_personnel_position_name || "category_personnel_position_name"} <span className='text-red-500'>*</span></label>
                            ...
                            {errName && <label className="text-sm text-red-500">{props.dataLang?.category_personnel_position_err_name || "category_personnel_position_err_name"}</label>}
                        </div>
                        <div className='space-y-1'>
                            ...
                            <Select 
                                ...
                                //     : null
                                // }
                                defaultValue={(position == "0" || !position) ? null : {label: dataOption.find(x => x?.value == position)?.label, value: position, level: dataOption.find(x => x.position_parent_id === position)?.level}}
                                value={(position == "0" || !position) ? null : {label: dataOptPosition.find(x => x?.value == position)?.label, value: position, level: dataOptPosition.find(x => x.value === position)?.level}}
