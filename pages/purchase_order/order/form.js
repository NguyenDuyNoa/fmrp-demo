...
const Index = (props) => {
    ...
    const [delivery_date, sDelivery_date] = useState(moment().format('YYYY-MM-DD HH:mm:ss'));
  ...
  useEffect(() => {
    ...
    const _HandleChangeInput = (type, value) => {
        if (type === "loai") {
            ...
            sDelivery_date(moment(value.target.value).format('YYYY-MM-DD HH:mm:ss'))
      ...
      const _ServerFetching =  () => {
        ...
        Axios("GET", "/api_web/Api_tax/tax?csrf_protection=true", {}, (err, response) => {
            if(!err){
                var {rResult} =  response.data
                sDataTasxes(rResult?.map(e =>({label: e.name, value: e.id, e})))       
            }
        })
    ...
    <div className='xl:px-10 px-3 xl:pt-24 pt-[88px] pb-3 space-y-2.5 flex flex-col justify-between'>
        <div className='h-[97%] space-y-3 overflow-hidden'>
            <div className='flex space-x-3 xl:text-[14.5px] text-[12px]'>
                <h6 className='text-[#141522]/40'>Đơn đặt hàng</h6>
                <span className='text-[#141522]/40'>/</span>
                <h6>Thông tin đơn đặt hàng</h6>
            </div>
            <div className='flex justify-between items-center'>
                <h2 className='xl:text-2xl text-xl '>Thông tin đơn đặt hàng</h2>
                    ...
                    <div className="flex flex-wrap justify-start gap-3 items-center mt-2"> 
                      ...
                      <label className="text-[#344054] font-normal text-sm mb-1 ">{"Nhà cung cấp"} <span className="text-red-500">*</span></label>
                        <Select 
                          ...
                          />
                          {errSupplier && <label className="text-sm text-red-500">{"Vui lòng nhập nhà cung cấp"}</label>}
                        </div>
                        <div className='w-[24.5%]'>
                          <label className="text-[#344054] font-normal text-sm mb-1 ">{"Nhân viên"} <span className="text-red-500">*</span></label>
                          <Select 
                            options={dataStaff}
                          ...
                          />
                        {errStaff && <label className="text-sm text-red-500">{"Vui lòng nhập nhân viên"}</label>}
                        </div>
                        <div className='w-[24.5%]'>
                            <label className="text-[#344054] font-normal text-sm mb-1 ">{"Ngày dự kiến giao hàng"} <span className="text-red-500">*</span></label>
                            <input
                              value={delivery_date}              
                        ...
                        <div className='w-[24.5%]'>
                          <label  className="text-[#344054] font-normal text-sm mb-1 ">{"Loại đặt hàng"} </label>
                          <div className='flex items-center gap-5'>
                              <div className="flex items-center ">
                                  ...
                                  <label for="default-radio-1" className="ml-2 cursor-pointer text-sm font-medium text-gray-900 dark:text-gray-300">Đặt mới</label>
                              </div>
                              <div className="flex items-center">
                                  ...
                                  <label for="default-radio-2" className="ml-2 cursor-pointer text-sm font-medium text-gray-900 dark:text-gray-300">Đặt theo YCMH</label>
                              </div>
                          </div>
                        </div>
                        {hidden && ( 
                          <div className='w-[24.5%]'>
                            <label className="text-[#344054] font-normal text-sm mb-1 ">{"Phiếu yêu cầu mua hàng"} <span className="text-red-500">*</span></label>
                              <Select 
                                  options={fakeDataPurchases}
                                  components={{ MultiValue }}
                                  onChange={_HandleChangeInput.bind(this, "purchases")}
                                  value={idPurchases}
                                  placeholder={"Phiếu yêu cầu mua hàng"} 
                                  hideSelectedOptions={false}
                                  isClearable={true}
                                ...
                                />
                              {errPurchase && <label className="text-sm text-red-500">{"Vui lòng chọn yêu cầu mặt hàng"}</label>}
                          </div>
                        )}
            ...
            <h2 className='font-normal bg-[#ECF0F4] p-2  '>{"Thông tin mặt hàng"}</h2>  
              ...
              <div className='grid grid-cols-12 items-center  sticky top-0  bg-[#F7F8F9] py-2 z-10'>
                  <h4 className='2xl:text-[14px] xl:text-[13px] text-[12px] px-2  text-[#667085] uppercase col-span-3      text-left    truncate font-[400]'>{"Mặt hàng"}</h4>
                  ...
                  <h4 className='2xl:text-[14px] xl:text-[13px] text-[12px] px-2  text-[#667085] uppercase  col-span-1    text-left    font-[400]'>{"Đơn giá sau chiết khấu"}</h4>
                  ...
                  <h4 className='2xl:text-[14px] xl:text-[13px] text-[12px] px-2  text-[#667085] uppercase  col-span-1    text-left   truncate font-[400]'>{"Thành tiền"}</h4>
                  <h4 className='2xl:text-[14px] xl:text-[13px] text-[12px] px-2  text-[#667085] uppercase  col-span-1    text-left     truncate font-[400]'>{"Ghi chú"}</h4>
        ...
        <h2 className='font-normal bg-[white]  p-2 border-b border-b-[#a9b5c5]  border-t border-t-[#a9b5c5]'>{"Tổng cộng"} </h2>  
        </div>
                ...
                <button
                 ...
                 className="button text-[#344054] font-normal text-base py-2 px-4 rounded-[5.5px] border border-solid border-[#D0D5DD]">{"Quay lại"}</button>
                  ...
                  onClick={_HandleSubmit.bind(this)} 
                   type="submit"className="button text-[#FFFFFF]  font-normal text-base py-2 px-4 rounded-[5.5px] bg-[#0F4F9E]">{"Lưu"}</button>
                </div>
            </div>
        </div>
