...
const Popup_chitietThere = (props) => {
    ...
    const _ServerFetching_detailThere = async () =>{
      await Axios("GET", `${props?.type == "import" && `/api_web/Api_import/import/${props?.id}?csrf_protection=true` || props?.type == "service" && `/api_web/Api_service/service/${props?.id}?csrf_protection=true`|| props?.type =="deposit" && `/api_web/Api_purchase_order/purchase_order/${props?.id}?csrf_protection=true`} `, {}, (err, response) => {
        if(!err){
              ...
              <h2 className='font-normal bg-[#ECF0F4] p-2 text-[13px]'>{props?.dataLang?.purchase_order_detail_general_informatione || "purchase_order_detail_general_informatione"}</h2>       
                ...
                <div className="pr-2 w-[100%] lx:w-[110%] ">
                  ...
                  {onFetching ?
                    ...
                    (<>
                        <ScrollArea     
                          className="min-h-[90px] max-h-[170px] 2xl:max-h-[166px] overflow-hidden"  speed={1}  smoothScrolling={true}>
