...
const Index = (props) => {
    ...
    const _ServerFetchingDetail = () => {
        Axios("GET", `/api_web/Api_delivery/get/${id}?csrf_protection=true`, {}, (err, response) => {
            if (!err) {
                ...
                const items = rResult?.items?.map((e) => ({
                    ...
                    price: +e?.price,
                    /// chiết khấu
                    discount: +e?.discount_percent_item,
                    ///Thuế
                    tax: { tax_rate: e?.tax_rate_item, label: e?.tax_rate_item, value: e?.tax_id_item },
                    //Đơn vị tính
                    unit: e.item?.unit_name,
                    /// đơn giá sau ck
                    priceAfterDiscount: +e?.price_after_discount,
                    //ghi chú
                    note: e?.note_item,
                    /// thành tiền
                    totalAmount: +e?.price_after_discount * (1 + +e?.tax_rate_item / 100) * +e?.quantity,
                }));
                setAddress({ label: rResult?.name_address_delivery, value: rResult?.address_delivery_id });
                setOption(items);
                setCodeDelivery(rResult?.reference_no);
                //người liên lạc
                setContactPerson(
                ...
                //ng dùng
                setStaff({
                    label: rResult?.staff_full_name,
                    value: rResult?.staff_id,
                });
                //khách hàng
                setCustomer({
                    label: rResult?.customer_name,
                    value: rResult?.customer_id,
                });
                setStartDate(moment(rResult?.date).toDate());
                setNote(rResult?.note);
                setProductOrder({
                    label: rResult?.order_code,
                    value: rResult?.order_id,
                });
            }
    ...
    const handlers = [
        { condition: onFetchingCustomer, FunctionFetch: handleFetchingCustomer },
        { condition: onFetchingStaff, FunctionFetch: handleFetchingStaff },
        { condition: onFetchingAddress, FunctionFetch: handleFetchingAddress },
        { condition: onFetchingQuote, FunctionFetch: handleFetchingProductOrder },
        { condition: onFetchingContactPerson, FunctionFetch: handleFetchingContactPerson },
    ];

    useEffect(() => {
        handlers.forEach(({ condition, FunctionFetch }) => {
            if (condition) {
                FunctionFetch();
            }
        });
    }, [onFetchingCustomer, onFetchingStaff, onFetchingAddress, onFetchingQuote, onFetchingContactPerson]);
    ...
    const options = dataItems?.map((e) => {
        return {
            label: `${e.name} <span style={{display: none}}>${e.item?.code}</span><span style={{display: none}}>${e.product_variation} </span><span style={{display: none}}>${e.text_type} ${e.unit_name} </span>`,
            value: e.id,
            e,
        };
    });
    ...
    useEffect(() => {
        ...
        setOption((prevOption) => {
            ...
            newOption.forEach((item, index) => {
                ...
                item.tax = totalTax;
                item.totalAmount = isNaN(thanhTien) ? 0 : thanhTien;
            });
            return newOption;
        });
    }, [totalTax]);
    ...
    useEffect(() => {
        ...
        setOption((prevOption) => {
            ...
            newOption.forEach((item, index) => {
                ...
                item.priceAfterDiscount = isNaN(dongiasauchietkhau) ? 0 : dongiasauchietkhau;
                item.totalAmount = isNaN(thanhTien) ? 0 : thanhTien;
            });
            return newOption;
        });
    ...
    const handleOnChangeInput = (type, value) => {
        if (type === "codeDelivery") {
            setCodeDelivery(value.target.value);
                ...
                const newData = value
                    ?.map((e, index) => {
                        return {
                            ...
                            priceAfterDiscount: +e?.e?.price * (1 - +e?.e?.discount_percent_item / 100),
                            tax: {
                                label: e?.e?.tax_name,
    ...
    const handleAddParent = (value) => {
        ...
        if (!checkData) {
            const newData = {
                ...
                priceAfterDiscount: +value?.e?.price * (1 - +value?.e?.discount_percent_item / 100),
                tax: {
            ...
            ToatstNotifi("error", `${"Mặt hàng đã được chọn"}`);
        }
    };
        ...
        if (type == "unit") {
            ...
            if (value?.floatValue > 0 && option[index].quantity > 0) {
                ...
                if (option[index].tax?.tax_rate == undefined) {
                    const tien =
                        Number(option[index].priceAfterDiscount) *
                        (1 + Number(0) / 100) *
                        Number(option[index].quantity);
                    option[index].totalAmount = Number(tien.toFixed(2));
                } else {
                    const tien =
                        Number(option[index].priceAfterDiscount) *
            ...
            option[index].priceAfterDiscount = +option[index].price * (1 - option[index].discount / 100);
            option[index].priceAfterDiscount = +(Math.round(option[index].priceAfterDiscount + "e+2") + "e-2");
            if (option[index].tax?.tax_rate == undefined) {
                const tien =
                    Number(option[index].priceAfterDiscount) * (1 + Number(0) / 100) * Number(option[index].quantity);
                option[index].totalAmount = Number(tien.toFixed(2));
            } else {
                const tien =
                    Number(option[index].priceAfterDiscount) *
    ...
    const handleIncrease = (id) => {
        ...
        if (newQuantity > +option[index].item.e.quantity) {
            ToatstNotifi("error", `${"Vượt quá số lượng tồn"}`);
        }
        if (newQuantity > +option[index].item.e.quantity - +option[index].item.e.quantity_delivery) {
            ToatstNotifi("error", `${"Vượt quá số lượng còn lại"}`);
        }
    };
    ...
    const handleDecrease = (id) => {
        ...
        if (newQuantity >= 1) {
            ...
            return ToatstNotifi("error", `${"Số lượng tối thiểu là 1 không thể giảm!"}`);
        }
    };
    const _HandleDelete = (id, type) => {
        if (type === "default") {
            return ToatstNotifi("error", `${"Mặc định của hệ thống, không thể xóa!"}`);
        }
    ...
    const handleSubmit = async () => {
        ...
        newDataOption.forEach((item, index) => {
            id && formData.append(`items[${index}][row_id]`, item?.idParentItem ? item?.idParentItem : "");
    ...
    const sortedArr = option.sort((a, b) => (id ? a.id - b.id : b.id - a.id));
                    ...
                    <div className="3xl:h-[330px] h-[400px] overflow-auto pb-2 scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100">
                        <div className="pr-2">
                            <div className="divide-y divide-slate-200 min:h-[400px] h-[100%] max:h-[800px]">
                                ...
                                <div className="grid grid-cols-12">
                                    ...
                                    <div className="grid col-span-9 grid-cols-9 gap-1  items-center mb-1">
                                        ...
                                        <div className="col-span-1 flex items-center justify-center">
                                            <button
                                                disabled={true}
                                                className="2xl:scale-100 xl:scale-90 scale-75 text-gray-400 hover:bg-[#e2f0fe] hover:text-gray-600 font-bold flex items-center justify-center p-0.5  bg-slate-200 rounded-full hover:scale-110 transition-all duration-200 ease-linear"
                                            >
                                                <Minus size="16" className="2xl:scale-100 xl:scale-90 scale-75 " />
                                            </button>
                                            <NumericFormat
                                                className={`cursor-default appearance-none text-center 3xl:text-[13px] 2xl:text-[12px] xl:text-[11px] text-[10px] py-1 px-0.5 font-normal 2xl:w-24 xl:w-[90px] w-[63px]  focus:outline-none border-b border-gray-200`}
                                                value={1}
                                                thousandSeparator=","
                                                allowNegative={false}
                                                readOnly={true}
                                        ...
                                        <div className="col-span-1 text-center flex items-center justify-center">
                                            <NumericFormat
                                                ...
                                                decimalScale={0}
                                                isNumericString={true}
                                                className={`cursor-default appearance-none 3xl:text-[13px] 2xl:text-[12px] xl:text-[11px] text-[10px] text-center py-1 px-2 font-normal w-[80%] focus:outline-none border-b border-gray-200`}
                                                thousandSeparator=","
                                            />
                                        </div>
                                        <div className="col-span-1 text-center flex items-center justify-center">
                                            <NumericFormat
                                                value={0}
                                                className={`cursor-default appearance-none text-center py-1 px-2 font-normal w-[80%] focus:outline-none border-b 3xl:text-[13px] 2xl:text-[12px] xl:text-[11px] text-[10px] border-gray-200`}
                                                thousandSeparator=","
                                                allowNegative={false}
                                                readOnly={true}
                                                isNumericString={true}
                                ...
                                {onFetchingDetail ? (
                                    <Loading className="max-h-60" color="#0f4f9e" />
                                ) : (
                                        ...
                                        <div className="grid grid-cols-12 gap-1 py-1 items-center" key={e?.id}>
                                            ...
                                            <div className="col-span-1 flex items-center justify-center">
                                                <div className="flex items-center justify-center">
                                                    ...
                                                    >
                                                        <Minus
                                                            size="16"
                                                            className="2xl:scale-100 xl:scale-90 scale-75"
                                                        />
                                                    </button>
                                                    <NumericFormat
                                                        className={`${
                                                            errQty && e?.quantity == ""
                                                                ? "border-red-400"
                                                                : "border-gray-200"
                                                        } cursor-text appearance-none text-center 3xl:text-[13px] 2xl:text-[12px] xl:text-[11px] text-[10px] py-1 px-0.5 font-normal 2xl:w-24 xl:w-[90px] w-[63px]  focus:outline-none border-b `}
                                                        onValueChange={(value) =>
                                                        ...
                                                        isAllowed={(values) => {
                                                            ...
                                                            if (floatValue > +e?.item?.e?.quantity) {
                                                                ToatstNotifi(
                                                                    "error",
                                                                    `Vui nhập số lượng nhỏ hơn hoặc bằng ${+e?.item?.e
                                                                        ?.quantity}`
                                                                );
                                                            }
                                                            if (floatValue > count) {
                                                                ToatstNotifi(
                                                                    "error",
                                                                    `Vui nhập số lượng nhỏ hơn hoặc bằng ${count}`
                                                                );
                                                            } else if (floatValue === 0) {
                                                                ToatstNotifi("error", `Vui nhập số lượng lớn hơn 0!`);
                                                            }
                                            ...
                                            <div className="col-span-1 text-center flex items-center justify-center">
                                                <NumericFormat
                                                    ...
                                                    decimalScale={0}
                                                    isNumericString={true}
                                                    className={`cursor-text appearance-none 3xl:text-[13px] 2xl:text-[12px] xl:text-[11px] text-[10px] text-center py-1 px-2 font-normal w-[80%] focus:outline-none border-b border-gray-200`}
                                                    thousandSeparator=","
                                                />
                                            </div>
                                            <div className="col-span-1 text-center flex items-center justify-center">
                                                <NumericFormat
                                                    ...
                                                    className={`cursor-text appearance-none text-center py-1 px-2 font-normal w-[80%]  focus:outline-none border-b 3xl:text-[13px] 2xl:text-[12px] xl:text-[11px] text-[10px] border-gray-200`}
                                                    thousandSeparator=","
                                                    allowNegative={false}
                                                    isAllowed={(values) => {
                                                        if (!values.value) return true;
                                            ...
                                            <div className="col-span-1 text-right flex items-center justify-end">
                                                ...
                                                >
                                                    {formatNumber(e?.priceAfterDiscount)}
                                                </h3>
                                            </div>
                        ...
                        <div className="col-span-2  flex items-center gap-2">
                            ...
                            <div className="col-span-1 text-center flex items-center justify-center">
                                <NumericFormat
                                    ...
                                    className="3xl:text-[18px] 2xl:text-[16px] xl:text-[14px] text-[12px] text-center py-1 px-2 bg-transparent font-normal xl:w-20 w-24 focus:outline-none border-b border-gray-300"
                                    thousandSeparator=","
                                    isAllowed={(values) => {
                                        if (!values.value) return true;
                                        const { floatValue } = values;
                                        if (floatValue > 101) {
                                            ToatstNotifi("error", `Vui lòng nhập số % chiết khấu nhỏ hơn 101`);
                                        }
                                        return floatValue < 101;
                                    }}
                                    allowNegative={false}
                        ...
                        <div className="flex justify-between ">
                            ...
                            <div className="font-normal 3xl:text-[18px] 2xl:text-[16px] xl:text-[14px] text-[13px]">
                                <h3 className="text-blue-600">
                                    {formatNumber(
                                        option.reduce((acc, item) => {
                                            const tienSauCK = item?.quantity * item?.priceAfterDiscount;
