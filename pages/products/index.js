  ...
  const multiDataSet = [
    {
      ...
      data: dataExcel?.map((e) => [
        ...
        {
          value: `${
            e.stock_quantity ? Number(e?.stock_quantity).toLocaleString() : ""
          }`,
        },
        { value: `${e.note ? e.note : ""}` },
    ...
    Axios(
      ...
      (err, response) => {
        if (!err) {
          ...
          sListCdChosen(
            data.map((e) => ({ label: e.stage_name, value: e.stage_id }))
          );
        }
        sOnFetching(false);
      }
    ...
    >
      <div className="py-4 w-[800px]">
        ...
        {onFetching ? (
          ...
          <>
            <ScrollArea
              className="min-h-[0px] max-h-[500px] overflow-hidden"
              speed={1}
              smoothScrolling={true}
              ref={scrollAreaRef}
            >
  ...
  const dataRestVariant = dataVariant?.filter(
    (item1) =>
      !dataSelectedVariant?.some(
        (item2) => item1.label === item2?.label && item1.value === item2?.value
      )
  );
          ...
          sDataSelectedVariant(
            data?.variations?.map((e) => ({
              ...
              child: e.items?.map((ce) => ({
                id: ce.id,
                type: { label: ce.str_type_item, value: ce.type_item },
                name: { label: ce.item_name, value: ce.item_id },
                unit: { label: ce.unit_name, value: ce.unit_id },
                norm: Number(ce.quota),
                loss: Number(ce.loss),
                stage: { label: ce.stage_name, value: ce.stage_id },
              })),
            }))
          );
          sCurrentData(
        ...
        if (!err) {
          ...
          sDataVariant(
            rResult[0]?.product_variation?.includes("NONE")
              ? [{ label: "Mặc định", value: rResult[0]?.id, child: [] }]
              : rResult.map((e) => ({
    ...
    if (type === "type") {
      ...
      if (found) {
        ...
        if (child) {
          ...
          Axios(
            "POST",
            "/api_web/api_product/searchItems?csrf_protection=true",
            {
              data: {
                type: type,
              },
          ...
          <>
            ...
            {dataSelectedVariant?.length > 0 && (
              <div className="pb-2 flex space-x-3 items-center justify-start overflow-auto scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100">
                {dataSelectedVariant.map((e) => (
                  ...
                  >
                    <span>
                      {e.label?.includes("NONE") ? "Mặc định" : e.label}
                    </span>
                    <button
                      type="button"
            ...
            <div className="space-y-1 -pt-5">
              <div className="flex w-full items-center bg-slate-100 p-2 z-10">
                <h4 className="xl:text-[13.5px] text-[12px] px-1 text-[#667085] uppercase w-[5%] font-[400] text-center">
                  {props.dataLang?.no || "no"}
                </h4>
                <h4 className="xl:text-[13.5px] text-[12px] px-1 text-[#667085] uppercase w-[37%] font-[400]">
                  {props.dataLang?.bom_name_finishedProduct}
                </h4>
                <h4 className="xl:text-[13.5px] text-[12px] px-1 text-[#667085] uppercase w-[15%] font-[400] text-center">
                  {props.dataLang?.unit}
                </h4>
                <h4 className="xl:text-[13.5px] text-[12px] px-1 text-[#667085] uppercase w-[10%] font-[400] text-left">
                  {props.dataLang?.norm_finishedProduct ||
                    "norm_finishedProduct"}
                </h4>
                <h4 className="xl:text-[13.5px] text-[12px] px-1 text-[#667085] uppercase w-[10%] font-[400] text-left">
                ...
                <h4 className="xl:text-[13.5px] text-[12px] px-1 text-[#667085] uppercase w-[23%] font-[400] text-left">
                  {props.dataLang?.stage_usage_finishedProduct ||
                    "stage_usage_finishedProduct"}
                </h4>
                <h4 className="xl:text-[13.5px] text-[12px] px-1 text-[#667085] uppercase w-[10%] font-[400] text-center">
                  {props.dataLang?.branch_popup_properties ||
                    "branch_popup_properties"}
                </h4>
              </div>
                  ...
                  {loadingData ? (
                    ...
                    <>
                      {selectedList?.child?.map((e, index) => (
                        <div
                          key={e.id}
                          className="py-1 px-2 flex w-full hover:bg-slate-100 items-center"
                        >
                          <h6 className="w-[5%] px-1 text-center">
                            {index + 1}
                          </h6>
                          ...
                          <Select
                            ...
                            className={`${
                              errName && e.name == null
                                ? "border-red-500"
                                : "border-transparent"
                            } Select__custom placeholder:text-slate-300 bg-[#ffffff] rounded text-[#52575E] font-normal outline-none border text-[13px] w-[15%]`}
                            theme={(theme) => ({
                              ...theme,
                              colors: {
                                ...theme.colors,
              ...
              {dataSelectedVariant?.length > 0 && (
                ...
                >
                  <IconAdd />
                  {props.dataLang?.bom_design_add_finishedProduct ||
