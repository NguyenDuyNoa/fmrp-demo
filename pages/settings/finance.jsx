...
const Index = (props) => {
    ...
    const _ServerFetching = async () => {
        const url = `/api_web/${(router.query?.tab === "taxes" && "Api_tax/tax?csrf_protection=true") ||
        ...
        try {
            const { rResult, output } = await apiFinance.apiListFinance(url, {
                params: {
                    search: keySearch,
                    limit: limit,
                                                            ...
                                                            {router.query?.tab === "paymentmodes" && (
                                                                <React.Fragment>
                                                                    ...
                                                                    <RowItemTable colSpan={1} textAlign={"right"}>
                                                                        {router.query?.tab === "paymentmodes" &&
                                                                            formatnumber(e?.opening_balance)}
                                                                    </RowItemTable>
                                                                    <RowItemTable colSpan={2}>
                                                                        {router.query?.tab === "paymentmodes" &&
                                                                            e?.description}
