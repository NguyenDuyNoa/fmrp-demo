...
const Popup_danhmuc = (props) => {
  ...
  // relative h-5 w-5 cursor-pointer appearance-none rounded-full border border-blue-gray-200 text-[#0F4F9E] transition-all before:absolute before:top-2/4 before:left-2/4 before:block before:h-12 before:w-12 before:-translate-y-2/4 before:-translate-x-2/4 before:rounded-full before:bg-blue-gray-500 before:opacity-0 before:transition-opacity checked:border-[#0F4F9E] checked:before:bg-[#0F4F9E] hover:before:opacity-10
...
const Popup_phuongthucthanhtoan = (props) => {
    ...
    const _HandleChangeInput = (type, value) => {
      if(type == "name"){
        ...
        sOpening_balance(value.target?.value)
