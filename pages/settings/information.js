    ...
    return(
        <React.Fragment>
            ...
            <div>
                ...
                <Btn_Setting url="/settings/general" isActive="/settings/general">Thiết lập chung</Btn_Setting>
            </div>
        </React.Fragment>
    )
})
...
const Btn_Setting = React.memo((props) => {
    ...
    return(
        <Link href={props.url ? props.url : "#"} alt={props.children}>
            <button className={`${router.asPath.includes(props.isActive) ? "text-white bg-[#11315B]" : "text-[#11315B] hover:bg-[#11315B]/5" } flex items-center space-x-2 rounded w-full text-left font-[400] py-2 px-3 my-1`}>
                <div className={`${router.asPath.includes(props.isActive) ? "bg-white" : "bg-[#11315B]" } w-1.5 h-1.5 rounded `} />
