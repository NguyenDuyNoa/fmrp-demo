...
const Index = (props) => {
  ...
  const _ServerFetching =  () =>{
    Axios("GET", `/api_web/Api_customers_groups/customers_groups?csrf_protection=true`, {
      params: {
        search: keySearch,
        limit: limit,
  ...
  const handleDelete = (event) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const id = event;
        Axios("DELETE", `/api_web/Api_customers_groups/customers_groups/${id}?csrf_protection=true`, {
        }, (err, response) => {
          if(!err){
                ...
                <div className="min:h-[200px] h-[82%] max:h-[500px] overflow-auto pb-2 scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100">
                  <div className="xl:w-[100%] w-[110%] pr-2">
                    <div className="flex items-center sticky top-0 bg-white p-2 z-10">
                      ...
                      <h4 className="xl:text-[14px] text-[12px] px-2 text-[#667085] uppercase w-[25%] font-[300] text-left">{dataLang?.client_group_color}</h4>
                      <h4 className="xl:text-[14px] text-[12px] px-2 text-[#667085] uppercase w-[25%] font-[300] text-center">{dataLang?.branch_popup_properties}</h4>
                    </div>
                    {onFetching ?
                        ...
                        <>
                          ...
                          {(data.map((e) => 
                            <div className="flex items-center py-1.5 px-2 hover:bg-slate-100/40 " key={e.id.toString()}>
                              ...
                              }} className="xl:text-base text-xs px-2 py-3 w-[25%] max-h-[40px] rounded-md ">{e.color}</h6>
                              <div className="space-x-2 w-[25%] text-center">
...
const Popup_groupKh = (props) => {
  ...
  const _ServerSending = () => {
    ...
    Axios("POST", `${props.id ? `/api_web/Api_customers_groups/customers_groups/${id}?csrf_protection=true` : "/api_web/Api_customers_groups/customers_groups?csrf_protection=true"}`, {
      data: data,
