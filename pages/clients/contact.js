...
const Index = (props) => {
    ...
    const listClient_filter = listClient?.map(e =>({label: e.name, value: e.id}))
    const [idClient, sIdClient] = useState(null);
    const hiddenOptionsClient = idClient?.length > 3 ? idClient?.slice(0, 3) : [];
    const optionsClient = listClient_filter ? listClient_filter?.filter((x) => !hiddenOptionsClient.includes(x.value)) : [];
    ...
    useEffect(() => {
        onFetching && _ServerFetching() || onFetching && _ServerFetching_brand()  || onFetching && _ServerFetching_client()
      }, [onFetching]);
    useEffect(() => {
         sOnFetching(true) || (keySearch && sOnFetching(true)) || (idBranch?.length > 0 && sOnFetching(true)) || idClient?.length > 0 && sOnFetching(true)
                ...
                <div className="xl:space-y-3 space-y-2">
                    ...
                    <div className='flex gap-2'>
                          ...
                          <div className='ml-1 w-[23vw]'>
                              <Select 
                                  // options={options}
                                  options={[{ value: '', label: 'Chi chọn nhánh', isDisabled: true }, ...options]}
                      ...
                      options={[{ value: '', label: 'Chọn khách hàng', isDisabled: true }, ...optionsClient]}
                ...
                <div className="min:h-[500px] 2xl:h-[90%] xl:h-[69%] h-[100%] max:h-[800px]  overflow-auto pb-2 scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100">
                  <div className="pr-2 w-[100%] lx:w-[110%] ">
                    <div className="flex items-center sticky top-0 bg-white p-2 z-10">
                      <h4 className="xl:text-[14px] text-[12px] px-2 text-[#667085] uppercase w-[15%] font-[300] text-left">{dataLang?.client_contact_table_fulname}</h4>
                      <h4 className="xl:text-[14px] text-[12px] px-2 text-[#667085] uppercase w-[20%] font-[300] text-left">{dataLang?.client_contact_table_name}</h4>
                      <h4 className="xl:text-[14px] text-[12px] px-2 text-[#667085] uppercase w-[15%] font-[300] text-center">{dataLang?.client_contact_table_phone}</h4> 
                      <h4 className="xl:text-[14px] text-[12px] px-2 text-[#667085] uppercase w-[15%] font-[300] text-left">{dataLang?.client_contact_table_mail}</h4>
                      <h4 className="xl:text-[14px] text-[12px] px-2 text-[#667085] uppercase w-[15%] font-[300] text-left">{dataLang?.client_contact_table_pos}</h4>
                      <h4 className="xl:text-[14px] text-[12px] px-2 text-[#667085] uppercase w-[15%] font-[300] text-left">{dataLang?.client_contact_table_hapy}</h4>
                      <h4 className="xl:text-[14px] text-[12px] px-2 text-[#667085] uppercase w-[15%] font-[300] text-left">{dataLang?.client_contact_table_address}</h4>
                      <h4 className="xl:text-[14px] text-[12px] px-2 text-[#667085] uppercase w-[15%] font-[300] text-left">{dataLang?.client_contact_table_brand}</h4>
                    </div>
                    {onFetching ?
                      ...
                      (<>
                          ...
                          {(data?.map((e) => 
                            <div className="flex items-center py-1.5 px-2 hover:bg-slate-100/40 " key={e.client_contact_id.toString()}>
                              <h6 className="xl:text-base text-xs  px-2 py-0.5 w-[15%]  rounded-md text-left">{e.client_name }</h6>                
                              <h6 className="xl:text-base text-xs  px-2 py-0.5 w-[20%]  rounded-md text-left">{e.contact_name}</h6>
                              ...
                              <h6 className="xl:text-base text-xs  px-2 py-0.5 w-[15%]  rounded-md text-left"><span className="flex flex-wrap justify-start ">{e?.branch?.map(e => (<span className="mr-2 mb-1 w-fit xl:text-base text-xs px-2 text-[#0F4F9E] font-[300] py-0.5 border border-[#0F4F9E] rounded-lg">{e.name}</span>))}</span></h6>                                 
