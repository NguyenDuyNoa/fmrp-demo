...
const FormContactDelivery = ({
  optionDelivery,
  dataLang,
  _OnChangeOptionDelivery,
  onDelete,
  children,
}) => {
  return (
          ...
          >
            ...
            <div>
              <span className="text-[#344054] font-normal text-sm group-hover:text-purple-500 transition-all duration-200 ease-linear">
                {dataLang?.client_popup_devivelyAddres ||
                  "client_popup_devivelyAddres"}
              </span>
            </div>
          </label>
        </div>
        <ButtoonDelete onClick={onDelete.bind(this, optionDelivery.id)}>
          {children}
        </ButtoonDelete>
      </div>
    </div>
