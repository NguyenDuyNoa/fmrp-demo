  ...
  const multiDataSet = [
    {
        columns: [
            ...
            {title: `${dataLang?.client_group_statusclient}`, width: {wpx: 100}, style: {fill: {fgColor: {rgb: "C7DFFB"}}, font: {bold: true}}},
            ...
            <div className="space-y-3 h-[96%] overflow-hidden">
              <div className="flex justify-between">
                  ...
                  <div className="flex justify-end items-center">
                    <Popup_status listBr={listBr} onRefresh={_ServerFetching.bind(this)} dataLang={dataLang} className="xl:text-sm text-xs xl:px-5 px-3 xl:py-2.5 py-1.5 bg-gradient-to-l from-[#0F4F9E] via-[#0F4F9E] via-[#296dc1] to-[#0F4F9E] text-white rounded btn-animation hover:scale-105" />
                  </div>
              </div>
                        ...
                        <>
                          ...
                          {(data?.map((e) => 
                            <div className="flex items-center py-1.5 px-2 hover:bg-slate-100/40 " key={e.id.toString()}>
                              ...
                              <h6 className="xl:text-base text-xs  px-2 py-3 w-[15%]  rounded-md  "><span className="flex flex-wrap justify-start space-x-2 ">{e?.branch?.map(e => (<span className="mb-1 w-fit xl:text-base text-xs px-2 text-[#0F4F9E] font-[300] py-0.5 border border-[#0F4F9E] rounded-lg">{e.name}</span>))}</span></h6>
                              <div className="space-x-2 w-[20%] text-center">
                                <Popup_status  listBr={listBr} sValueBr={e.branch} onRefresh={_ServerFetching.bind(this)} className="xl:text-base text-xs " dataLang={dataLang} name={e.name} color={e.color} id={e.id} />
