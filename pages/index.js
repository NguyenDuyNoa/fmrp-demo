...
const DualAxes = dynamic(() => import('@ant-design/plots').then(({ DualAxes }) => DualAxes),
    { ssr: false }
);
      ...
      <div className='py-3 divide-y divide-slate-200'>
        {data.map((e) => 
          <div className='flex items-center justify-between py-2 hover:bg-slate-100/30' key={e.name}>
            ...
            <div className={`${e.percent >= 50 ? "bg-[#EBF5FF]" : "bg-[#f6e8cd6d]"} w-[28%] relative h-2 rounded-full bg-[#EBF5FF]`}>
              <div className={`${e.percent >= 50 ? "bg-[#5599EC]" : "bg-[#FF8F0D]"} h-2 rounded-full absolute left-0`} style={{width: `${e.percent}%`}} />
            </div>
            <h6 className={`${e.percent >= 50 ? "text-[#5599EC] bg-[#EBF5FF] border-[#d4e8fd]" : "text-[#FF8F0D] bg-[#FEF8EC] border-[#f6e8cd]"} py-2 w-[10%] text-center rounded-md border`}>{e.percent}%</h6>
          </div>
        )}
      </div>
    </div>
      ...
      <div className='py-3 divide-y divide-slate-200'>
        {data.map((e) => 
          <div className='flex items-center justify-between py-2 hover:bg-slate-100/30' key={e.name}>
            ...
            <h6 className={`${(e.status === 1 && "text-[#0BAA2E] bg-[#EBFEF2]") || (e.status === 2 && "text-[#FF8F0D] bg-[#FEF8EC]") || (e.status === 3 && "text-[#EE1E1E] bg-[#FFEEF0]")} py-2 px-3 text-center rounded-md`}>
