...
const ResultChatBot = ({ productAnalysis, onRetry, onRedirect }) => {
    return (
        <div className="w-full max-w-[473px] mx-auto px-1 py-5 text-center flex flex-col items-center justify-center gap-y-8">
            {/* Tiêu đề + icon */}
            ...
            {/* Mô tả + link */}
            {productAnalysis.isSuccess === 1 || productAnalysis.isSuccess === true ? (
                <p className="text-lg font-deca font-medium text-[#1C252E]">
            ...
            {/* Hành động */}
            {productAnalysis.isSuccess === 1 || productAnalysis.isSuccess === true ? (
                <div className="flex justify-center gap-x-2 flex-row">
                    <ButtonAnimationNew
                        title="Phân tích sản phẩm khác"
                        className="flex items-center gap-2 bg-white px-3 py-2 border border-[#00A76F] text-[#00A76F] rounded-xl hover:bg-[#F3F4F6] transition text-lg font-deca font-normal hover:shadow-hover-button "
                        onClick={() => onRetry()}
                    />
                    ...
                    <ButtonAnimationNew
                        ...
                        title=" Truy cập ngay "
                        className="flex items-center gap-2 bg-linear-background-toggle-bot rounded-xl px-3 py-2 text-white transition text-lg font-deca font-medium hover:shadow-hover-button "
                        onClick={() => onRedirect()}
                        reverse={1}
                    />
                </div>
