...
const Popup_status = (props) => {
    ...
    const { values, columns } = useMemo(() => {
        ...
        if (props.router == 1) {
            const allFields = [...props.listData, ...props.listDataContact, ...props.listDataDelivery];
            ...
            const mappedData = newArr.map((item) => {
                ...
                allFields.forEach((column) => {
                    ...
                    rowData[column.dataFieldsContact?.value] = item[column.dataFieldsContact?.value] || "";
                    rowData[column.dataFieldsDelivery?.value] = item[column.dataFieldsDelivery?.value] || "";
                });
                return rowData;
            });
            ...
            const values = mappedData.map((i) =>
                allFields.map((header) => {
                    const value =
                        ...
                        i[header.value] ||
                        (header.dataFieldsContact?.value === "contact" ? JSON.stringify(i.contact) : "") ||
                        (header.dataFieldsDelivery?.value === "arrAddress" ? JSON.stringify(i.arrAddress) : "");
            ...
            const mappedData = newArr.map((item) => {
                ...
                allFields.forEach((column) => {
                    const value =
                        item[column.dataFieldsContact?.value] || item[column.dataFields?.value] || item[column?.value];
                    rowData[column.dataFields?.value] = value || "";
                    rowData[column.dataFieldsContact?.value] = item[column.dataFieldsContact?.value] || "";
                });
                return rowData;
            ...
            const values = mappedData.map((i) =>
                Object.values(i)?.map((e) => ({
                    value: e,
                    style: e == "" ? { fill: { patternType: "solid", fgColor: { rgb: "FFCCEEFF" } } } : "",
                }))
            );

            return { values, columns };
        }
    }, [props.data, props?.listData, props.listDataContact, props.listDataDelivery]);
                ...
                <div className="min:h-[200px] h-[82%] max:h-[500px]  overflow-auto pb-2 scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100">
                    <div className="flex items-center justify-between p-1 bg-gray-50">
                        <div className="flex items-center gap-2">
                            ...
                            <FilterRemove size="20" color="red" className="transition-all animate-pulse" />
                        </div>
                        {props?.router != 5 && (
                            <ExcelFile
                    ...
                    <div className="pr-2 w-[100%] lx:w-[110%] ">
                        <div className={`grid-cols-12  grid sticky top-0 bg-white shadow-lg  z-10`}></div>
                        {data_ex?.length > 0 ? (
                            <>
                                        ...
                                        {data_ex?.map((e) => (
                                            ...
                                            >
                                                <h6 className="text-[13px] col-span-12    py-2.5 text-left flex items-center gap-1">
                                                    ...
                                                    />
                                                    <h6 className="font-semibold text-blue-500">Dòng {e?.rowIndex}</h6>
                                                    <h6>-</h6>
