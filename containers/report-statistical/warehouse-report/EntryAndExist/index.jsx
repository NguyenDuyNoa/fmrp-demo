...
import { useExportExcel } from '../ExportDelivery/hook/useExportExcel'
import { useGetListReportImportFinishedGoods } from '../ImportGoods/hook/useGetListReportImport'
  ...
  return (
    <ReportLayout
          ...
          renderScrollableRow={(item, index) => (
            <>
              ...
              <RowItemTable className="w-32 flex justify-center items-center py-2 px-3 border-r border-[#E0E0E1] !text-new-blue font-normal flex-shrink-0">
                {Number(item.in_qty) === 0 ? '-' : formatNumber(Number(Math.abs(item.in_qty || 0)))}
              </RowItemTable>
              <RowItemTable className="w-32 flex justify-center items-center py-2 px-3 border-r border-[#E0E0E1] text-neutral-07 font-normal flex-shrink-0">
              ...
              {/* Xuất Kho */}
              <RowItemTable className="w-32 flex justify-center items-center py-2 px-3 border-r border-[#E0E0E1] !text-new-blue font-normal flex-shrink-0">
                {Number(item.out_qty) === 0 ? '-' : formatNumber(Number(Math.abs(item.out_qty || 0)))}
              </RowItemTable>
              <RowItemTable className="w-32 flex justify-center items-center py-2 px-3 border-r border-[#E0E0E1] text-neutral-07 font-normal flex-shrink-0">
          ...
          renderFooter={() => (
            <>
              ...
              <RowItemTable className="h-10 w-32 flex items-center justify-center px-3 text-neutral-07 font-semibold flex-shrink-0 bg-white">
                {Number(dataReportStock?.rTotal?.opening_qty) === 0 ? '-' : formatNumber(Number(dataReportStock?.rTotal?.opening_qty))}
              </RowItemTable>
              <RowItemTable className="h-10 w-32 flex items-center justify-center px-3 text-neutral-07 font-semibold flex-shrink-0 bg-white">
      ...
      totalSection={
        <Pagination
          postsPerPage={limit}
          totalPosts={Number(dataReportStock?.output?.iTotalDisplayRecords) || 0}
          paginate={paginate}
