            ...
            >
                <div className="w-auto">
                    <div className="bg-white py-2 px-0.5 rounded-t justify-between flex divide-x divide-[#DDDDE2]">
                        {props.data?.map((e) => 
                            <div className={`${e.title ? "px-7 py-3" : "px-1"} space-y-4 min-w-[150px]`} key={e.title.toString()}>
                                {e.title && <h3 className="text-[20px] font-medium px-3">{e.title}</h3>}
                                {e.sub?.map((ce) => 
                                    <div className="space-y-0.5" key={ce.title.toString()}>
                                        {ce.link ? 
                                            <Link title={ce.title} href={ce.link} className="flex items-center space-x-2 mb-2 px-3 py-2 rounded hover:bg-[#ececee87] text-[#344054]">
                                                <Image src={ce?.img} width={18} height={18} quality={100} className="object-contain" loading="lazy" crossOrigin="anonymous" blurDataURL="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" />
                                        ...
                                        {ce.items?.map((e) => 
                                            <Link href={e.link ? e.link : "#"} title={e.name} className="outline-none" key={e.name.toString()}>
