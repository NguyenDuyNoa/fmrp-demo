...
const TagColorSky = ({ name, className }) => {
    return <span className={`${className} font-normal responsive-text-sm text-[#C25705] bg-[#FF811A26] rounded py-0.5 px-1.5 2xl:py-1 2xl:px-2 w-fit`}>
        {name}
    </span>
}

const TagColorOrange = ({ name, className, ...props }) => {
    return <span {...props} className={`${className} text-center font-normal responsive-text-sm text-[#1A7526] bg-[#35BD4B33] rounded py-0.5 px-1.5 2xl:py-1 2xl:px-2 w-fit`}>
        {name}
    </span>
}

const TagColorLime = ({ name, className }) => {
    return <span className={`${className} flex responsive-text-sm items-center gap-1 font-normal text-[#1A7526] bg-[#35BD4B33] rounded py-0.5 px-1.5 2xl:py-1 2xl:px-2 w-fit`}>
        {/* <TickCircle
...
const TagColorRed = ({ name, className }) => {
    return <span className={`${className} font-normal responsive-text-sm text-red-500 rounded py-0.5 px-1.5 2xl:py-1 2xl:px-2 w-fit bg-red-200`}>
        {name}
    </span>
}
...
const TagColorMore = ({ name, className, backgroundColor, color }) => {
    return <span
        ...
        className={`${className} font-normal responsive-text-sm rounded py-0.5 px-1.5 2xl:py-1 2xl:px-2 w-fit `}>
        {name}
    </span>
}
